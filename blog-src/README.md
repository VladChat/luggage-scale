...
| `description` | One or two sentences (155â€“160 chars is ideal). | `<meta name="description">`, Open Graph/Twitter descriptions, teaser copy on listings. |
| `socialImage` | Absolute URL or a `/blog/...` path to a 1200Ã—630 image. | Open Graph/Twitter preview image. Falls back to `config.seo.defaultOgImage`. |
...
- `metaTitle` / `metaDescription` â€“ override the computed title or description for the `<head>` tags when you need custom copy.
- `ogType` â€“ defaults to `article` on posts and `website` on listings. Override for landing pages that need a different Open Graph type.
- `twitterCard` â€“ defaults to `summary_large_image`; set to `summary` if you deliberately ship a smaller card.
- `metaRobots` â€“ supply custom robots directives (e.g., `noindex`) instead of using the automatic `published: false` guard.
- `socialImageAlt` â€“ short alt text for the social sharing image, applied to the `og:image:alt` and `twitter:image:alt` tags.
- `authorTwitter` / `twitterCreator` â€“ handle shown in the `twitter:creator` tag when the post has an author-specific account.
...
- Omit the optional `--date` flag to use todayâ€™s date in the generated folder names and front matter, or provide an explicit ISO date: `npm run generate:posts -- ./topics.csv --date=2024-05-20`.
